...
addLoadEvent(function() {
    var fieldsets = getElementsByTagAndClassName('fieldset', null, 'questionnaire');

    // Grab the legends
    var legends = getElementsByTagAndClassName('legend', null, 'questionnaire');
    ...
    forEach(legends, function(legend) {
        ...
        if (row) {
            fsid = getNodeAttribute(row, 'id').replace(/^questionnaire_(.*)description_container$/, '$1');
        }
        a.id = fsid + '_a';
    ...
    //removeElement('questionnaire_topsubmit_container');

    // last part is the submit buttons
    appendChildNodes('questionnaire',
        tabUL, DIV({'class': 'questionnaire-fieldsets subpage'}, fieldsets), getFirstElementByTagAndClassName('td', null, 'questionnaire_submit_container').childNodes
    );
    removeElement(
        getFirstElementByTagAndClassName('table', null, 'questionnaire')
    );

    // Connect events to each form element to check if they're changed and set
    // a dirty flag
    var formDirty = false;
    forEach(getElementsByTagAndClassName(null, null, 'questionnaire'), function(i) {
    ...
    hideElement('tab1-loading');
    $('questionnaire').style.position = 'static';
...
var rule = '#questionnaire { visibility: hidden; position: absolute; top: 0; }';
