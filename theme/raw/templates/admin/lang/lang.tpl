{include file="header.tpl"}
        ...
        <div id="adminlang">
            <form id="fileselectionform" method="get">
                <select name="lang" onchange="$('fileselectionform').submit()">
                ...
                <select name="file" onchange="$('fileselectionform').submit()">
                    {foreach from=$lang_files key=path item=name}
                        <option value="{$path}"{if $path eq $current_file} selected="selected"{/if}>{$name}</option>
                    {/foreach}
                </select>
			...
			<table cellspacing="0" cellpadding="1" id="strings" wrap="yes">
                ...
                {foreach from=$lang_strings key=stringid item=string}
                    <tr class="{cycle name=rows values=r1,r0}">
                        ...
                        <textarea name="strings[{$string->elementid}]" rows="3" cols="40" id="text_{$string->elementid}" onChange="queue_string('{$string->elementid}')" onBlur="string_blur('{$string->elementid}')" onFocus="string_focus('{$string->elementid}')">{$string->translated}</textarea>
